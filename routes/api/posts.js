...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    if (post.user.toString() !== req.user.id) {
      return res.status(401).json({ mssg: 'User not authorized' });
    }
...
router.put('/unlike/:id', auth, async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({ mssg: 'Post has not been liked' });
    }
...
router.delete('/comment/:id/:comment_id', auth, async (req, res) => {
  try {
    ...
    if (!comment) {
      return res.status(404).json({ mssg: 'Comment not found' });
    }
